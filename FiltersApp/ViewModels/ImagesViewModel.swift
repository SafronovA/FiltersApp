...
protocol ImagesViewModelProtocol: AnyObject {
    var imageItems: [ImageItem]? { get }
    var imageEntities: [Entity]? { get }
    var imageItemsExist: Bool { get }
    var imageEntitiesExist: Bool { get }
...
class ImagesViewModel: ImagesViewModelProtocol {
    ...
    var imageItemsExist: Bool {
        return imageItems?.isEmpty == false
    }
    
    var imageEntitiesExist: Bool {
        return imageEntities?.isEmpty == false
    }
    
    var imageEntities: [Entity]? {
        didSet {
            if((self.imageEntities?.isEmpty == false) && (self.imageItems?.isEmpty == true)){
    ...
    var imageItems: [ImageItem]? {
        didSet {
            if(self.imageItems?.isEmpty == false){
                self.imageEntities = nil
                self.cleanCoreData()
                self.imagesDidChange?(self)
            }
        }
    ...
    func entityBy(objectID: NSManagedObjectID) -> Entity? {
        return viewContext.object(with: objectID) as? Entity
    }
    
    private func loadImageEntitiesFromCoreData(){
        do {
            self.imageEntities = try self.viewContext.fetch(Entity.fetchRequest())
    ...
    private func cleanCoreData(){
        do {
            try self.viewContext.execute(Entity.batchDeleteRequest())
    ...
    //        DispatchQueue.global().async { [weak self] in
    //            guard let self = self else {return}
    //            for im in imageURLsWithSize {
    //                //                self.ref.child(UUID().uuidString).setValue(ImageItem(url: im).toAnyObject())
    //                let url = im[0] as! String
    //                let width = im[1] as! Int16
    //                let height = im[2] as! Int16
